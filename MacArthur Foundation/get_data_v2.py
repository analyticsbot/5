...
if debug:
    sys.stdout.write( 'All modules loaded. Starting the scraping.' + '\n')
...
def getFellowData(id, valid_chars):
    ...
    if debug:
        sys.stdout.write( 'Visiting url - ' +  url + '\n')
    resp = requests.get(url)
    if resp.status_code == 200:
        if debug:
            sys.stdout.write('Success!! Received a 200 response from the server for fellow id :: ' + str(id) + '\n')
    else:
        if debug:
            sys.stdout.write( 'Failure!! Didnt receive 200 response from the server for fellow id :: ' + str(id)+ '\n')
    
    if 'Sorry' in unidecode(resp.text):
        if debug:
            sys.stdout.write( 'No more fellows to scrape. Exiting'+ '\n')
    ...
    if debug:
        sys.stdout.write( 'Fellow name is -- ' + unidecode(fellow_name))
    ...
    if debug:
        sys.stdout.write( 'Successfully scraped data for :: ' + unidecode(fellow_name)+ \
    ...
    if debug:
        sys.stdout.write( 'Successfully written data for'+ unidecode(fellow_name)+ ' to file -- '+ \
    ...
    try:
        ...
        if debug:
            sys.stdout.write( 'Successfully written media data for'+ unidecode(fellow_name)+ ' to file  -- '+ \
                  file_name)
            sys.stdout.write('')
            sys.stdout.write('###########################################################')
